...
function addBee() {
  let nbBees = document.getElementById("nbBees").value++;
  document.getElementById("nbBees").innerText = nbBees;
...
function updateBees() {
  ...
  if (score < 1000) updateTimer = setTimeout("updateBees();", period);
  else {
    clearTimeout();
    alert("Game over!");
  }
...
function isHit(defender, offender) {
  if (overlap(defender, offender)) {
    ...
    if (moved) {
      ...
      if (isNaN(longestDuration)) longestDuration = 0; //fixed an error in the code which would cause this code to become NaN constantly
